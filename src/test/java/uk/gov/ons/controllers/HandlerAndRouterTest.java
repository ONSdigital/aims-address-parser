...
class HandlerAndRouterTest {
    ...
    private final Logger logger = LoggerFactory.getLogger(HandlerAndRouterTest.class);

    private final String address = "Acme Flowers Ltd First And Second Floor Flat 39b Cranbrook Road Windleybury GU166DE";
    ...
    void testParseAddress() {
        client.get()
                ...
                .consumeWith(response -> {
                    ...
                    logger.info(new String(response.getResponseBody()));
                })
    ...
    @Test
    void testParseAddressWhitespaceOnly() {
        client.get()
                .uri(uriBuilder -> uriBuilder.path("/tokens")
                        .queryParam("address", "   ").build())
                .exchange()
    ...
    @Test
    void testParseAddressWithSpecialCharacters() {
        client.get()
                .uri(uriBuilder -> uriBuilder.path("/tokens")
                        .queryParam("address", "Café Münchën #42, Résumé St.").build())
                .exchange()
                .expectStatus().isOk()
                .expectBody()
                .consumeWith(response -> {
    ...
    @Test
    void testParseAddressMalformedQuery() {
        client.get()
                .uri("/tokens?address=")
                .exchange()
                .expectStatus().isBadRequest()
